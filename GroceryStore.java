...
public class GroceryStore implements Serializable {
	...
	public Result addProduct(Request request) {
		...
		if (!products.isProduct(request.getProductID())) {
			product = new Product(request.getProductName(), Integer.valueOf(request.getProductReorderLevel()),
					0, Double.valueOf(request.getProductPrice()));
			ProductList.getInstance().insertProduct(product);
